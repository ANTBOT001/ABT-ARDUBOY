...
/*  int adc_key_in = analogRead(A0);
  if (adc_key_in < 50)        button[1] = 0;//  right
  else if (adc_key_in < 195)  button[3] = 0; //up
  else if (adc_key_in < 380)  button[2] = 0; //Down
  else if (adc_key_in < 555)  button[4] = 0; //left
  else if (adc_key_in < 790)  button[0] = 0;*///select
  if (!digitalRead(A1))  {  
    delay(10);  if(!digitalRead(A1))  button[1] = 0;  
    }else if (!digitalRead(A0))  {delay(10);  if(!digitalRead(A0))  button[3] = 0;
    }else if (!digitalRead(A3))  {delay(10);  if(!digitalRead(A3))  button[2] = 0;
    }else if (!digitalRead(A2))  {delay(10);  if(!digitalRead(A2)) button[4] = 0; 
    }else if (!digitalRead(7))   {delay(10);  if(!digitalRead(7)) button[0] = 0;
    }
